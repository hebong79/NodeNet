...
export class Lobby {
 ...
 // 클라이언트에게 데이터를 브로드캐스트
  BroadcastInRoom( client:net.Socket, data:Buffer, roomName:string, isAll:boolean=false) {
  ...
  Receive_UserInfo(  client:net.Socket, data : Buffer) {
    ...
    let kUser = new SOUser(info.userId, info.ip, info.publicIp, info.dataPort, info.movePort);
    kUser.socket = client;
