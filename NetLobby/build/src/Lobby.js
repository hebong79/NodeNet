...
const PacketDatas = __importStar(require("./PacketDatas"));
...
class Lobby {
    ...
    // 클라이언트에게 데이터를 브로드캐스트
    BroadcastInRoom(client, data, roomName, isAll = false) {
    ...
    Receive_CreateId(client, data) {
        ...
        let nSuccess = PacketDatas_1.ECreateId.Success;
        //존재하면 실패처리
        if (resUser != undefined) {
            nSuccess = PacketDatas_1.ECreateId.Fail;
        }
        // 결과 전송
    ...
    Receive_UserInfo(client, data) {
        ...
        let kUser = new PacketDatas_1.SOUser(info.userId, info.ip, info.publicIp, info.dataPort, info.movePort);
        kUser.socket = client;
        this.lobbyUserList[info.userId] = kUser;
        console.log(`lobbyUserList count = ${Object.keys(this.lobbyUserList).length}\n`);
    }
    // 탈퇴 하기 응답
    Receive_Withdraw(client, data) {
    ...
    // 초기 룸리스트 응답 
    Receive_InitRoomList(client, data) {
    ...
    // 룸생성 응답
    Receive_CreateRoom(client, data) {
        ...
        const room = new PacketDatas_1.SORoom(roomName, userId, 4);
    ...
    Receive_JoinRoom(client, data) {
        ...
        if (!(roomName in this.roomList)) {
            ...
            const room = new PacketDatas_1.SORoom(roomName, userId);
    ...
    // 룸에서 Ready 응답
    Receive_RoomUserReady(client, data) {
    ...
    // 서버연결 끊기 응답
    Receive_Disconnect(client, data) {
